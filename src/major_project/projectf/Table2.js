...
const ChartDataget = () => {
  ...
  const getProduct = async () => {
    ...
    let result = await fetch("http://3.111.144.19:5000/getitemchart",{
        headers:{authorization:JSON.parse(localStorage.getItem('authentication'))
    }});
  ...
  const hendelerDelete=async(id)=>{
    try{
    let result=await fetch (`https://jwt-token-eight.vercel.app/deletechartitem/${id}`,{
      method:"Delete",
      headers: {
  ...
  return (
    ...
    <Link to="/Create" style={{textDecoration:"none"}}>Add Data</Link>

      </div>
      <div className="create" style={{    marginLeft: "122px"}}>
    <Link to="/Maindash" style={{textDecoration:"none"}}>Home</Link>
 ...
 border:"1px solid black",
      ...
      >
        ...
        {Product.map((item, index) => (
          <ul style={{ marginLeft:"-31px",borderBottom: "1px solid gray"}} key={item._id}  >
            <li
               style={{
                display: "inline-block",
            ...
            <li style={{
                ...
                <Link to ={"/updatechartdata/"+item._id}><AiIcon.AiFillEdit/></Link>
            </li>
            <li style={{
                display: "inline-block",
                border: "1px solid white",
                ...
                <Link><AiIcon.AiOutlineDelete  onClick={()=>hendelerDelete(item._id)} /></Link>
            </li>
          </ul>
        ))}
